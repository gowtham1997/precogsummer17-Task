...
def data():
    ...
    '''
        trump_popularity - sum of unique user tweets, number of retweets of 
                           users using hashtags stored in trump_hashtags
        hillary_popularity- sum of unique user tweets, number of retweets of 
                            users using hashtags stored in hillary_hashtags
        trump_set          - set which stores unique user ids of tweets 
                             with trump_hashtags
        hillary_set          - set which stores unique user ids of tweets with 
                               hillary_hashtags
    ...
    hillary_hashtags = ['#clinton', '#hillary',
                        '#imwithher', '#strongertogether']

    for tweet in tweets:

        #: tweet_text,tweet_image,tweet_image_and_text - boolean flags to
        #  identify type of tweet
        tweet_text = tweet_image = tweet_image_and_text = False

        tweet_string = tweet['text']

        #: tweet_string_parsed stores tweet text without urls, hashtags and
        #  mentions
        tweet_string_parsed = ''

        user_id = tweet['user']['id']

        #: RT_status - boolean flag indicating whether tweet is a
        #  Retweet/original
        RT_status = False
        ...
        for tweet_word in tweet_string.split():
            ...
            #: checking if string is a hashtag
                ...
                '''
                    tweet_string_parsed stores what's left in the tweet after
                    removing mentions,hashtags and urls.
                '''
                tweet_string_parsed = tweet_string_parsed.strip() + ' '
                + tweet_word
        ...
        for hashtag_record in tweet['entities']['hashtags']:
            #: words are encoded in utf-8 and lowercased to avoid redundant
            #: hashtags like #hillary and #Hillary
        ...
        '''
            ...
            Note: 'geo' field is deprecated. Ref: https://dev.twitter.com/
                                                            overview/api/tweets

            Users tweets with geo enabled are very rare and streaming for
            geo_enabled tweets does'nt yield fruitful results.
        '''
        if(tweet['coordinates'] is not None):
            ...
            '''
                Approximate user location can be calculated by assuming the
                tweet's location to user's location(given at time of creation).

                Drawback: Some people type imaginary locations - Heaven, hell
                          ,etc and the api takes alot of time to process all
                          tweets
            '''
    ...
    '''
        tweet_type_frequency stores frequency of type of tweet(text,images and
         both)
    '''

    tweet_type_frequency = []
    tweet_type_frequency.extend(
        (text_counter, image_counter, text_and_image_counter))
